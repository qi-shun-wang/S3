...
public extension S3 {
    ...
    func put(string: String, mime: MediaType, destination: String, bucket: String?, access: AccessControlList, on container: Container) throws -> Future<File.Response> {
    ...
    func put(string: String, mime: MediaType, destination: String, access: AccessControlList, on container: Container) throws -> Future<File.Response> {
        return try put(string: string, mime: mime, destination: destination, bucket: nil, access: access, on: container)
    }
    
    /// Upload file content to S3
    func put(string: String, destination: String, access: AccessControlList, on container: Container) throws -> Future<File.Response> {
        return try put(string: string, mime: .plainText, destination: destination, bucket: nil, access: access, on: container)
    }
    
    /// Upload file content to S3
    func put(string: String, mime: MediaType, destination: String, on container: Container) throws -> Future<File.Response> {
        return try put(string: string, mime: mime, destination: destination, access: .privateAccess, on: container)
    }
    
    /// Upload file content to S3
    func put(string: String, destination: String, on container: Container) throws -> Future<File.Response> {
